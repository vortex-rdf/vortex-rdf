...
#
#   * `build:fast` (wasm-pack --no-opt) skips wasm-opt, which dominates the
#     full build's wall clock while costing no extra memory. The trade is that
#     the tests then run against an unoptimized binary, so CI stays the source
#     of truth for the exact artifact that ships.
#
#   * CARGO_BUILD_JOBS is capped. Memory here is spent by rustc, not wasm-opt:
#     a warm incremental rebuild is one process, but a cold wasm build compiles
#     ~900 dependency crates and cargo would otherwise fan out to one rustc per
#     core — which is what makes this job unrunnable on a small machine.
#     ci.yml pins it to 1 for 4-core/16 GB runners; 4 is the local compromise.
#     Override by exporting CARGO_BUILD_JOBS yourself.
#
# Rebuilds js/pkg/web in place (gitignored, so the tree stays clean), replacing
# any wasm-opt'd build already there.
