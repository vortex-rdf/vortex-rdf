...
| `npm run bench:arrow` | What does the Arrow surface cost per consumer step — a zero-copy view, its copy, its IPC bytes, the reads after? | printed |
...
| `compare.worker.ts` | One adapter's full lifecycle in its own process, so a crash loses only that adapter's rows and its peak RSS is attributable to it alone. One role per process: `query`, `querycold`, `fullscan`, `mutate`, and `arrow` — the last for the adapters that export Arrow, spawned with `--experimental-wasm-rab-integration` so its views are zero-copy, and kept out of the other roles' processes because `apache-arrow` and that flag would move every reading beside them |
...
| `arrow.bench.ts` | Prices the Arrow surface per consumer step (tinybench): `matchArrow` views (zero-copy under the resizable-buffer flag the script passes), `toTable()` copies and `toIPC()` bytes, then `toArray()`/value reads on top, per encoding over a point pattern, a predicate pattern and the full scan; asserts the views survive wasm memory growth |
...
npm run bench:arrow                         # the Arrow surface, zero-copy views
